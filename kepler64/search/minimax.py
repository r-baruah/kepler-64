...
physics: candidate batches are statically padded to the theoretical maximum
         (218) and evaluated with one consistent vmap shape.
...
def negamax(engine, board: FastBoard, depth: int, alpha: float, beta: float,
    ...
    if depth <= 1:
        return _quiesce(engine, board, alpha, beta, current_mv, qdepth=3,
                        parent_masses=parent_masses, stats=stats)
    ...
    if allow_null and depth >= 3 and not board.in_check(board.turn == 0):
        ...
        null_val = -negamax(engine, null_board, depth - 3, -beta, -beta + 1,
                             ...
                             parent_masses=current_mv, stats=stats)
        if null_val >= beta:
            return beta

    best = -INF
    for m in _ordered_moves(board, consts, current_mv):
        child = board.apply(m)
...
def best_move(engine, board: FastBoard, depth: int = 3):
    ...
    for m in moves:
        child = board.apply(m)
        mv = child_mass_vector(board, m, parent_mv)
        val = -negamax(engine, child, depth - 1, -beta, -alpha, mv,
                       parent_masses=parent_mv)
        if val > best:
            best, bm = val, m
        if best > alpha:
            alpha = best
